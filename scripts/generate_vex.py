...
def __format_pull_request(cve_id: str, solr_version: str, project_label: str, vex_data: list[list[Any]]) -> str:
    result_lines = []
    result_lines.append(f"Analyse impact of {cve_id} on {project_label} {solr_version}\n")
...
def __undetermined_entry(artifact_purl: PackageURL, cve_id: str, project_label: str, solr_version: str) -> Any:
    """Return an entry when reachability cannot be determined."""
    return {
        "ids": [cve_id],
        "versions": solr_version,
        "jars": [f"{artifact_purl.name}-{artifact_purl.version}.jar"],
        "analysis": {
            "state": UNDETERMINED_STATE,
            "detail": f"{cve_id} in {artifact_purl.name}@{artifact_purl.version}: reachability could not be determined for {project_label}. A maintainer must complete this assessment."
        }
    }
...
def __generate_vex_input_entry(vex_data: list[list[Any]], artifact_purl: PackageURL, cve_id: str, solr_version: str, project_label: str) -> Any:
    ...
    for chain_data in vex_data:
        ...
        if not state:
            entries.append(__undetermined_entry(artifact_purl, cve_id, project_label, solr_version))
            continue
        if state == "unaffected":
            state = "not_affected"
        summary = f"{cve_id} in {artifact_purl.name}@{artifact_purl.version} is {"NOT reachable" if state == "not_affected" else "REACHABLE"} in {project_label}."
        entries.append({
...
if __name__ == "__main__":
    ...
    project_label = f"Apache {sbom.metadata.component.name}" if sbom.metadata.component.name else "Apache Logging"
    ...
    try:
        ...
        print(f"WARNING: VEX engine failed ({exc}); degrading to {UNDETERMINED_STATE}", file=sys.stderr)
        vex_data = []

    # Write PR content to temporary file
    if vex_data:
        pr_text = __format_pull_request(cve_id, solr_version, project_label, vex_data)
    else:
        pr_text = f"Analyse impact of {cve_id} on {project_label} {solr_version}\n\nReachability analysis could not be completed automatically. A maintainer must review and complete this VEX statement."
